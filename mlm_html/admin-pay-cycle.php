...
{ 
	...
	$sql=  "SELECT 	id 	FROM {$table_prefix}mlm_users WHERE id IN(SELECT parent_id AS id FROM {$table_prefix}mlm_commission WHERE payout_id =0 UNION SELECT sponsor_id AS id FROM {$table_prefix}mlm_referral_commission 	WHERE payout_id =0 UNION SELECT mlm_user_id AS id FROM {$table_prefix}mlm_bonus WHERE payout_id =0 )";
